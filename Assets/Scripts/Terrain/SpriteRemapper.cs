	...
	private Texture2D WrapRectangularTextureToCircle(Texture2D initialTexture) {
		...
		for (int y = 0; y < circleTexture.height; ++y) {
			...
			for (int x = 0; x < circleTexture.width; ++x) {
				...
				squareX01 = phi / (2 * PI) + 0.75f;

				initialTextureX = FloorToInt(squareX01 * initialTexture.width);
				initialTextureY = FloorToInt(squareY01 * initialTexture.height);
	...
	private Texture2D WrapCircularTextureToSquare(Texture2D circleTexture) {
		...
		for (int y = 0; y < outputTexture.height; ++y) {
			...
			for (int x = 0; x < outputTexture.width; ++x) {
				...
				if (Abs(unitCircleXY.y - center.y) > Abs(unitCircleXY.x - center.x)) {
					// Chunk 1
				...
				circleTextureX = FloorToInt(targetPointUnitSquare.x * circleTexture.width);
				circleTextureY = FloorToInt(targetPointUnitSquare.y * circleTexture.height);
